...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    ...
    $OutputDirectory = Join-Path $projectRoot (
        "Reports\GpuVisibleTiles\ab-$timestamp-device-$DeviceIndex-cameras-$CameraCount")
}
...
$variantDefinitions = @(
    ...
    [pscustomobject]@{ Name = 'wave-tile-32'; Expected = 'WaveTile32' },
    [pscustomobject]@{ Name = 'wave-tile-64'; Expected = 'WaveTile64' }
)
$rows = [System.Collections.Generic.List[object]]::new()

for ($round = 1; $round -le $Repeats; $round++) {
    switch (($round - 1) % 4) {
        0 { $roundVariants = @($variantDefinitions[0], $variantDefinitions[1], $variantDefinitions[2], $variantDefinitions[3]) }
        1 { $roundVariants = @($variantDefinitions[1], $variantDefinitions[2], $variantDefinitions[3], $variantDefinitions[0]) }
        2 { $roundVariants = @($variantDefinitions[2], $variantDefinitions[3], $variantDefinitions[0], $variantDefinitions[1]) }
        3 { $roundVariants = @($variantDefinitions[3], $variantDefinitions[0], $variantDefinitions[1], $variantDefinitions[2]) }
    }
    ...
    foreach ($variant in $roundVariants) {
        ...
        if ($variant.Name -like 'wave-tile-*' -and $report.bfp2WaveTileCullSupported -ne '1') {
            throw "$stem requested a visible-tile kernel, but runtime tile support is unavailable."
        }
        ...
        $row = [pscustomobject]@{
            ...
            ValidationVisibleTiles = [long]$report.bfp2ValidationVisibleTiles
            ValidationPaddedDrawIndices = [long]$report.bfp2ValidationPaddedDrawIndices
            ValidationAvoidedLogicalIndexBytes = [long]$report.bfp2ValidationAvoidedLogicalIndexBytes
...
$aggregateRows = foreach ($variant in $variantDefinitions) {
    ...
    [pscustomobject]@{
        ...
        VisibleTilesMedian = Get-Median @($variantRows | ForEach-Object ValidationVisibleTiles)
        PaddedDrawIndicesMedian = Get-Median @($variantRows | ForEach-Object ValidationPaddedDrawIndices)
        AvoidedLogicalIndexBytesMedian = Get-Median @($variantRows | ForEach-Object ValidationAvoidedLogicalIndexBytes)
    }
}
...
$summary.Add('# GPU No-copy Visible Tiles A/B')
$summary.Add('')
...
$summary.Add('')
$summary.Add('| Variant | Visible tiles | Padding indices | Avoided logical index bytes |')
$summary.Add('|---|---:|---:|---:|')
foreach ($row in $aggregateRows) {
    $summary.Add((
        '| {0} | {1:F0} | {2:F0} | {3:F0} |' -f
        $row.Variant,
        $row.VisibleTilesMedian,
        $row.PaddedDrawIndicesMedian,
        $row.AvoidedLogicalIndexBytesMedian))
}
$summary.Add('')
$summary.Add(
    'Avoided logical bytes count the removed full-index UAV write and vertex-stage reread. ' +
    'They are not a measured DRAM-bandwidth claim.')
$summary.Add('')
...
Write-Host "Completed counterbalanced GPU no-copy visible-tile A/B runs: $outputRoot"
