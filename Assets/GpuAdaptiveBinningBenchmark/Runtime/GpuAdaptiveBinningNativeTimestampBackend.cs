...
internal sealed class GpuAdaptiveBinningNativeTimestampBackend : IDisposable
{
    private readonly GpuTimestampSession session;
    private readonly CommandBuffer frequencyCommands;
    private bool disposed;

    private GpuAdaptiveBinningNativeTimestampBackend(
        GpuTimestampSession session)
    {
        this.session = session;
        frequencyCommands = new CommandBuffer
        {
            name = "GPU.AdaptiveBinning/NativeTimestamp/Frequency"
        };
    ...
    public static bool TryCreate(
        out GpuAdaptiveBinningNativeTimestampBackend backend,
        out GpuTimestampSupport support)
    {
        ...
        try
        {
            backend = new GpuAdaptiveBinningNativeTimestampBackend(session);
            return true;
        }
        catch
        {
    ...
    private void ThrowIfDisposed()
    {
        if (disposed)
        {
            throw new ObjectDisposedException(
                nameof(GpuAdaptiveBinningNativeTimestampBackend));
        }
    }
}
