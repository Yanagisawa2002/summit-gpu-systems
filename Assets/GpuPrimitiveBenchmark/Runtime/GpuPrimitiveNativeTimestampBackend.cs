...
internal sealed class GpuPrimitiveNativeTimestampBackend : IDisposable
{
    private readonly GpuTimestampSession session;
    private readonly CommandBuffer frequencyCommands;
    private bool disposed;

    private GpuPrimitiveNativeTimestampBackend(GpuTimestampSession session)
    {
        this.session = session;
        frequencyCommands = new CommandBuffer
        {
            name = "GPU.Primitives/NativeTimestamp/Frequency"
        };
    ...
    public static bool TryCreate(
        out GpuPrimitiveNativeTimestampBackend backend,
        out GpuTimestampSupport support)
    {
        backend = null;
        if (!GpuTimestampSession.TryCreate(out GpuTimestampSession session, out support))
        {
            return false;
        }
        ...
        try
        {
            backend = new GpuPrimitiveNativeTimestampBackend(session);
            return true;
        }
        catch
        {
    ...
    {
        ThrowIfDisposed();
        GpuTimestampScope scope = session.Scopes[scopeIndex];
    ...
    private void ThrowIfDisposed()
    {
        if (disposed)
        {
            throw new ObjectDisposedException(nameof(GpuPrimitiveNativeTimestampBackend));
        }
    }
}
