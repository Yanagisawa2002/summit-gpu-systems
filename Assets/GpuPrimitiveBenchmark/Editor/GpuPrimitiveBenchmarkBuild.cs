...
public static class GpuPrimitiveBenchmarkBuild
{
    private const string TemporaryScenePath =
        "Assets/GpuPrimitiveBenchmark/Generated/GpuPrimitiveBenchmark.generated.unity";
    ...
    public static void PerformBuild()
    {
        ...
        Directory.CreateDirectory(Path.GetDirectoryName(playerPath) ??
            throw new InvalidOperationException("Player output directory is invalid."));

        string generatedDirectory = Path.GetDirectoryName(TemporaryScenePath)?
            .Replace('\\', '/');
        if (!AssetDatabase.IsValidFolder(generatedDirectory))
        {
            AssetDatabase.CreateFolder(
                "Assets/GpuPrimitiveBenchmark",
                "Generated");
        }
        ...
        try
        {
            ...
            Scene scene = EditorSceneManager.NewScene(
                ...
                NewSceneMode.Single);
            scene.name = "GpuPrimitiveBenchmark";

            GameObject cameraHost = new GameObject("Benchmark Camera");
            ...
            cameraHost.transform.position = new Vector3(0.0f, 0.0f, -10.0f);
            ...
            {
                "GPU primitive benchmark Player build",
            ...
            {
                throw new InvalidOperationException(
                    "GPU primitive benchmark Player build failed: " +
