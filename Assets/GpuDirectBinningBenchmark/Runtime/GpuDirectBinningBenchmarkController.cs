...
public sealed class GpuDirectBinningBenchmarkController : MonoBehaviour
{
    ...
    private readonly List<GpuDirectBinningValidationResult> validationResults =
        new List<GpuDirectBinningValidationResult>(4);
    private readonly List<PendingTimestamp> pendingTimestamps =
        new List<PendingTimestamp>(256);

    private GpuDirectBinningBenchmarkAdapter adapter;
    private GpuDirectBinningNativeTimestampBackend timestampBackend;
    ...
    private static void Bootstrap()
    {
        ...
        GameObject host = new GameObject(
            "GPU Direct Binning Benchmark Controller");
        DontDestroyOnLoad(host);
        GpuDirectBinningBenchmarkController controller =
            host.AddComponent<GpuDirectBinningBenchmarkController>();
    ...
    private void Configure(string[] args)
    {
        ...
        superRounds = Mathf.Clamp(
            ReadInt(
                args,
                "-gpu-direct-binning-super-rounds",
                superRounds),
            1,
            4);
        warmupFrames = Mathf.Clamp(
            ReadInt(
                args,
                "-gpu-direct-binning-warmup-frames",
                warmupFrames),
            5,
            1800);
        sampleFrames = Mathf.Clamp(
            ReadInt(
                args,
                "-gpu-direct-binning-sample-frames",
                sampleFrames),
            60,
            7200);
        cooldownFrames = Mathf.Clamp(
            ReadInt(
                args,
                "-gpu-direct-binning-cooldown-frames",
                cooldownFrames),
            0,
            600);
        ...
        binCount = Mathf.Clamp(
            ReadInt(
                args,
                "-gpu-direct-binning-bin-count",
                binCount),
            1,
            16776960);
        seed = ReadInt(args, "-gpu-direct-binning-seed", seed);
        ...
        buildCommit = ReadString(
            args,
            "-gpu-direct-binning-build-commit",
            buildCommit);
    ...
    private IEnumerator Run()
    {
        ...
        try
        {
            adapter = new GpuDirectBinningBenchmarkAdapter(
                elementCount,
                binCount,
                distribution,
                seed,
        ...
        IReadOnlyList<GpuDirectBinningScheduleEntry> schedule =
            GpuDirectBinningBenchmarkSchedule.Build(superRounds);
        for (int scheduleIndex = 0; scheduleIndex < schedule.Count; scheduleIndex++)
        {
            GpuDirectBinningScheduleEntry entry = schedule[scheduleIndex];
            ...
            for (int sample = 1; sample <= sampleFrames; sample++)
            {
                ...
                GpuTimestampSampleFlags flags =
                    entry.Variant == GpuDirectBinningBenchmarkVariant.Control
    ...
    private void InitializeTimestampBackend()
    {
        timestampOperational =
            GpuDirectBinningNativeTimestampBackend.TryCreate(
        ...
        Action<bool> completion)
    {
        ...
        GpuDirectBinningValidationResult timeout =
            new GpuDirectBinningValidationResult
            {
                Phase = phase,
    ...
    private IEnumerator CompleteWithGraphicsFence(Action<bool> completion)
    {
        ...
        CommandBuffer commands = new CommandBuffer
        {
            name = "GPU.DirectBinning/CompletionFence"
        };
    ...
    private BlockSummary SummarizeBlock(
        GpuDirectBinningScheduleEntry entry,
        int sampleStart,
        int count,
        bool fencePassed)
    {
        ...
        GpuDirectBinningBenchmarkVariant variant = entry.Variant;
        return new BlockSummary
        {
            ...
            DispatchesPerFrame =
                variant == GpuDirectBinningBenchmarkVariant.Control
                    ? 0
                    : dispatchesPerFrame,
    ...
    private void WriteValidationResults()
    {
        using (StreamWriter writer = CreateWriter("validation.csv"))
        {
            ...
            foreach (GpuDirectBinningValidationResult row in validationResults)
            {
                writer.WriteLine(string.Join(",",
                    Csv(row.Phase),
                    Csv(row.CaseId),
    ...
    private void WriteRunSummary(bool passed, string status)
    {
        ...
        foreach (GpuDirectBinningValidationResult result in validationResults)
        {
        ...
        string[] lines =
        {
            "GPU direct spatial binning benchmark",
            "schemaVersion=1",
            "suite=summit.gpu-direct-binning",
            "passed=" + (passed ? "1" : "0"),
            "status=" + status,
    ...
    private void Finish(bool passed, string status)
    {
        ...
        finished = true;
        Debug.Log(
            "GPU direct-binning benchmark " + status +
    ...
    private static int CaseIndex(GpuDirectBinningBenchmarkVariant variant)
    {
        return variant == GpuDirectBinningBenchmarkVariant.Control
            ? 0
    ...
    private static GpuDirectBinningBenchmarkVariant VariantFromCaseIndex(
        int caseIndex)
    {
        return caseIndex == 0
            ? GpuDirectBinningBenchmarkVariant.Control
