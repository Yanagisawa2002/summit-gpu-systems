...
internal sealed class GpuDirectBinningNativeTimestampBackend : IDisposable
{
    private readonly GpuTimestampSession session;
    private readonly CommandBuffer frequencyCommands;
    private bool disposed;

    private GpuDirectBinningNativeTimestampBackend(
        GpuTimestampSession session)
    {
        this.session = session;
        frequencyCommands = new CommandBuffer
        {
            name = "GPU.DirectBinning/NativeTimestamp/Frequency"
        };
    ...
    public static bool TryCreate(
        out GpuDirectBinningNativeTimestampBackend backend,
        out GpuTimestampSupport support)
    {
        ...
        try
        {
            backend = new GpuDirectBinningNativeTimestampBackend(session);
            return true;
        }
        catch
        {
    ...
    private void ThrowIfDisposed()
    {
        if (disposed)
        {
            throw new ObjectDisposedException(
                nameof(GpuDirectBinningNativeTimestampBackend));
        }
    }
}
