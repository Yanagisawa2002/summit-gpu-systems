...
internal sealed class GpuDrivenInstanceNativeTimestampBackend : IDisposable
{
    private readonly GpuTimestampSession session;
    private readonly CommandBuffer frequencyCommands;
    private bool disposed;

    private GpuDrivenInstanceNativeTimestampBackend(
        GpuTimestampSession session)
    {
        this.session = session;
        frequencyCommands = new CommandBuffer
        {
            name = "GPU.DrivenInstance/NativeTimestamp/Frequency"
        };
    ...
    public static bool TryCreate(
        out GpuDrivenInstanceNativeTimestampBackend backend,
        out GpuTimestampSupport support)
    {
        ...
        try
        {
            backend = new GpuDrivenInstanceNativeTimestampBackend(session);
            return true;
        }
        catch
        {
    ...
    private void ThrowIfDisposed()
    {
        if (disposed)
        {
            throw new ObjectDisposedException(
                nameof(GpuDrivenInstanceNativeTimestampBackend));
        }
    }
}
