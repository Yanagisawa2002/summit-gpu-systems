...
def recorded_allocation_total(result, frames):
    if not result.get('allocationCounterAvailable',False) or any(f['recordAllocatedBytes']<0 for f in frames):
        return None
    return sum(f['recordAllocatedBytes'] for f in frames)
...
def analyze(root, output):
    ...
    for entry in read(root/'matrix.json'):
        ...
        cells.append(dict(id=entry['id'], scene=cfg['scenario'], replicate=cfg['processReplicate'], passed=not errors,
                          ...
                          gc=[dict(arm=a['arm'],block=a['block'],allocationCounterAvailable=result.get('allocationCounterAvailable',False),recordAllocatedBytes=recorded_allocation_total(result,a['frames']),gc0=a['frames'][-1]['gc0']-a['frames'][0]['gc0'],gc1=a['frames'][-1]['gc1']-a['frames'][0]['gc1'],gc2=a['frames'][-1]['gc2']-a['frames'][0]['gc2']) for a in result['runs']]))
