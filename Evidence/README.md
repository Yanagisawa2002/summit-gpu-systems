...
- GPU-driven instances and visible-only scatter: `../Docs/GPU_DRIVEN_VISIBLE_ONLY_NVIDIA_RTX4090_FORMAL_2026-08-29.md`
...
Retained hardware results cover only the device named in each report. GPU primitives and device-keyed autotuning include AMD Radeon AI PRO R9700 and NVIDIA GeForce RTX 4090 Direct3D 12 evidence. The new GPU-driven visible-only result is NVIDIA-only; AMD is unavailable for that workload. Percentages describe the exact workloads, scopes, sample counts, and baselines in their reports; they are not generalized product-wide speedups.
